...
enum ButtonPagePositon {
  left,
  right,
  middle,
}

class ButtonScaffoldController extends ValueNotifier<ButtonPagePositon> {
  ButtonScaffoldController([
    ButtonPagePositon value = ButtonPagePositon.middle,
  ]) : super(value);

  Future? animateToPage(ButtonPagePositon pagePositon) {
    return _onAnimateToPage?.call(pagePositon);
  }

  Future? animateToLeft() {
    return _onAnimateToPage?.call(ButtonPagePositon.left);
  }

  Future? animateToRight() {
    return _onAnimateToPage?.call(ButtonPagePositon.right);
  }

  Future? animateToMiddle() {
    return _onAnimateToPage?.call(ButtonPagePositon.middle);
  }

  Future Function(ButtonPagePositon pagePositon)? _onAnimateToPage;
}

class ButtonScaffold extends StatefulWidget {
  final ButtonScaffoldController? controller;
  ...
  @override
  ButtonScaffoldState createState() => ButtonScaffoldState();
}

class ButtonScaffoldState extends State<ButtonScaffold>
  ...
  Future animateToPage(p) async {
    ...
    switch (p) {
      case ButtonPagePositon.left:
        await animateTo(screenWidth!);
        break;
      case ButtonPagePositon.middle:
        await animateTo();
        break;
      case ButtonPagePositon.right:
        await animateTo(-screenWidth!);
        break;
    }
      ...
      children: <Widget>[
        ...
        _MiddlePage(
          ...
          onTopDrag: () {
            // absorbing = true;
            setState(() {
              debugPrint('*******--------***********  absorbing = $absorbing ');
            });
          },
          offsetX: offsetX,
          offsetY: offsetY,
  ...
  onHorizontalDragEnd(details, screenWidth) {
    ...
    if (vOffset > scrollSpeed && inMiddle == 0) {
      // 去右边页面
      return animateToPage(ButtonPagePositon.left);
    } else if (vOffset < -scrollSpeed && inMiddle == 0) {
      // 去左边页面
      return animateToPage(ButtonPagePositon.right);
    } else if (inMiddle > 0 && vOffset < -scrollSpeed) {
      return animateToPage(ButtonPagePositon.middle);
    } else if (inMiddle < 0 && vOffset > scrollSpeed) {
      return animateToPage(ButtonPagePositon.middle);
    }
  ...
  Future animateToTop() {
    ...
    animationY = Tween(begin: offsetY, end: 0.0).animate(curve)..addListener(() {
      setState(() {
  ...
  Future animateTo([double end = 0.0]) {
    ...
    animationX = Tween(begin: offsetX, end: end).animate(curve)..addListener(() {
      setState(() {
  ...
  void calculateOffsetY(DragUpdateDetails details) {
    ...
    debugPrint('absorbing.toString() = ${absorbing.toString()}');
  }
...
class _MiddlePage extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    Widget tabBarContainer = tabBar ?? SizedBox(
